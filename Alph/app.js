angular.module('alphApp', []).controller('AlphController', ['$scope', function($scope) {
    ...
    $scope.update = function update () {
        $scope.encripted = doCrypt();
        $scope.decripted = doCrypt(true);
    }
    ...
    function doCrypt(isDecrypt) {
        ...
        if (isDecrypt) {
            for (var i = 0; i < key.length; i++)
                key[i] = (26 - key[i]) % 26;
            return crypt($scope.encripted, key);
        }
        return crypt($scope.input, key);
    }
